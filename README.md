...
`ETL_TAG` | Used to specify a modality-specific subprocess (e.g. for DICOM data, CODEX/MACSima, or light-sheet microscopy) **(Optional)**. The following are valid examples of values: <br> `dicom-vol` (DICOM Volume) <br> `highly-mti` (Highly Multiplexed Tissue Imaging) <br> `big-hist` (Large Image of Histology Slide) <br> `lsfm-vol` (Light-Sheet Fluorescence Microscopy) 
