...
def process(transaction):
	...
	for e in experiments:
		if e.getExperimentType() == "Q_EXT_NGS_QUALITYCONTROL":
			exp = e
	if not exp:
		numberOfExperiments = len(experiments) + 1
		exp = transaction.createNewExperiment('/' + space + '/' + project + '/' + project + 'E' + str(numberOfExperiments), "Q_EXT_NGS_QUALITYCONTROL")

        newSample = transaction.createNewSample('/' + space + '/' + 'NGSQC'+ parentCode, "Q_EXT_NGS_QUALITYCONTROL_RUN")
        newSample.setParentSampleIdentifiers([sa.getSampleIdentifier()])
      
	newSample.setExperiment(exp) 
        # create new dataset 
        dataSet = transaction.createNewDataSet("Q_EXT_NGS_QUALITYCONTROL_RESULTS")
