...
def process(transaction):
    ...
    #Register files
    dataSetRes = transaction.createNewDataSet('Q_WF_NGS_EPITOPE_PREDICTION_RESULTS')
    dataSetRes.setMeasuredData(False)
    dataSetLogs = transaction.createNewDataSet('Q_WF_NGS_EPITOPE_PREDICTION_LOGS')
    ...
    for root, subFolders, files in os.walk(incomingPath):
        for f in files:
            if f.endswith('info'):
                os.rename(os.path.join(root, f), os.path.join(incomingPath + '/logs', f))
